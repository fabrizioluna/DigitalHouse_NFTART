...
// Todos los Usuarios
   users: function(req, res){
       User.findAll({
        attributes: { exclude: ['contrasenia', 'imagen'] }}).then(function(users){
           res.json({
...
// Un Usuario por ID quitando datos sensibles
    userId: function(req, res){
        User.findByPk(req.params.id, {
            attributes: { exclude: ['contrasenia', 'imagen'] }}   ).then(function(user){
            res.json({
            description: "User Id",
            code: 200,
            user
...
// Servicio que retorne los productos (.count), (.countbycategory), (.productos  datos de cada producto)
    product: async function(req,res){
        ...
        let productsPicture = await nft.count({where:{categoria:"Fotografia"}})
        let productsMusic = await nft.count({where:{categoria:"Musica"}})
        let productsArt = await nft.count({where:{categoria:"Arte"}})
        let productsMetaverso = await nft.count({where:{categoria:"Metaverso"}})
    ...
    productId: async function (req, res){
        let productId = await nft.findOne({where:{id:req.params.id}, include:[{model: User, as: "Creador"}, {model: Category, as: "Categoria"}, {model: Autores, as: "Autor"}]})
        res.json({
            description: "Product ID",
            code: 200,
            productId
